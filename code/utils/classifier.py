...
class moeLoRAClassifier(nn.Module):
    """
    A classifier to select LoRA layers for moeLoRA.
    """
    ...
    def __init__(
        ...
        self.n_layers = n_layers  # 所有LoRA的需要缩放的层数
        self.config = config
        ...
        if self.config.moelora_depth == 1:
        ...
        elif self.config.moelora_depth == 2:
            self.inner.append(
                nn.Linear(config.hidden_size, config.moelora_size, bias=bias_flag).to(config.device).to(dtype)
            )
            
            if config.enable_relu_and_dropout:
                self.inner.append(nn.ReLU())
                self.inner.append(nn.Dropout(p=config.moelora_dropout_p))
            
            if config.layerwise_scalings:
                self.last = (
                    nn.Linear(config.moelora_size, n_classes * n_layers, bias=bias_flag).to(config.device).to(dtype)
                )
            else:
                self.last = nn.Linear(config.moelora_size, n_classes, bias=bias_flag).to(config.device).to(dtype)
        else:
            assert self.config.moelora_depth > 0
            self.inner.append(
                nn.Linear(config.hidden_size, config.moelora_size, bias=bias_flag).to(config.device).to(dtype)
            )
            ...
            for _ in range(config.moelora_depth - 2):
                self.inner.append(
                    nn.Linear(config.moelora_size, config.moelora_size, bias=bias_flag).to(config.device).to(dtype)
                )
    ...
    def forward(
        ...
        **kwargs,
        ...
        logger.info("*** Running moeLoRAClassifier forward ***")
            ...
            with model.disable_adapter():
                ...
                result: ModelOutput = model.forward(
                    *args,
                    ...
                    _moelora_classifier_inhibitor_flag=InhibitorFlagPayload(
        ...
        logger.info(
            f"batch_size: {batch_size}, seq_len: {seq_len}, n_layers: {self.n_layers}, n_classes: {self.n_classes}")
        # logger.info(f"get_maybe_topk_scalings-----moelora_scalings: {scalings}")
        # logger.info('---------------before softmax---------------')
        # logger.info(scalings)
        ...
        if self.n_predictions_lifetime > 0:
            ...
            self.n_predictions_lifetime = self.n_predictions_lifetime - 1
    ...
    def get_nb_trainable_parameters(self):
        ...
        for _, param in self.named_parameters():
            ...
            all_param = all_param + num_params
    ...
    def _save_scalings(file: str, scalings: List[torch.Tensor]):
        ...
        result_cpu = result.cpu().detach().float()  # 将 Tensor 从 GPU 复制到 CPU
        npy = result_cpu.numpy()  # 将 Tensor 转换为 NumPy 数组并保存
        numpy.save(file, npy)
